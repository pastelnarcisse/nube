 ...
 */
	 ...
	 * @return boolean|integer
	 */
	public function dataModule($identityColumn='', $identity='')
	{
		if (empty($identity))
		{
			return false;
		...
		$builder = $this->db->table($this->tables['module']);

		$query = $builder->select($this->tables['module'].'.* ')
						 ->select($this->tables['menu'].'.link menuLink')
						 ->select($this->tables['menu'].'.name menuName')
						 ->where($this->tables['module'].'.'.$identityColumn, $identity)
						 ->join('menu', $this->tables['module'].'.men_id = '.$this->tables['menu'].'.men_id','left')
						 ->limit(1)
						 ->get()
						 ->getRow();
	...
	/**
	 * Checar si el modulo puede acceder a cierta página
	 *
	 * @param integer|array $checkGroup Group(s) to check
	 * @param integer       $id         User id
	 * @param boolean       $checkAll   Check if all groups is present, or any of the groups
	 ...
	 * @author Phil Sturgeon
	 **/
	public function inModule(int $id=0)
	{
		$this->triggerEvents('inModule');
		...
		foreach ($usersGroups as $group){
			...
			array_push($modules, ...$this->permissionGroup($group->id)->result());
	...
	/**
	* Obten los permisos del módulo
	*
	* @param integer $groupID id de grupo
	* @param string $moduleLink nombre del módulo
	* @param string $actionName nombre de la acción.
	*
	* @return static
	* @author Chununo
	*/
	public function permissionGroup(int $groupID = 0, string $moduleLink = '', string $actionName = ''){

		$this->triggerEvents('permissionGroup');

		if ($groupID) {
			$this->where($this->tables['permissionGroup'] . '.groupID', $groupID);
			$this->where($this->tables['permissionGroup'] . '.permission', 1);
			$this->orderBy('menuOrder', 'desc');
			$this->orderBy('moduleOrder', 'desc');
		}

		
		if ($moduleLink != '') {
			$this->where($this->tables['permissionGroup'] . '.moduleLink', $moduleLink);
			$this->where($this->tables['permissionGroup'] . '.menuStatus', 1);
			$this->where($this->tables['permissionGroup'] . '.moduleStatus', 1);
		}

		if ($actionName != '') {
			$this->where($this->tables['permissionGroup'] . '.actionName', $actionName);
			$this->limit(1);
		}
	...
	/**
	* Permiso de acciones del grupo
	*
	* @return static
	* @author Chununo
	*/
	public function permissionGroups(): self {
	
		$this->triggerEvents('permissionGroups');

		$builder = $this->db->table($this->tables['permissionGroup']);
		...
		if (isset($this->ionOrWhere) && ! empty($this->ionOrWhere))	{
			foreach ($this->ionOrWhere as $orWhere)	{
	...
	*
	* @param integer $groupID id de grupo
	* @param string $menuLink controlador/funcion
	*
	* @return static
	* @author Chununo
	*/
	public function permissionGroupModule(int $groupID = 0, string $menuLink = ''){

		$this->triggerEvents('permissionGroupModule');

		if ($groupID) {
			$this->where($this->tables['permissionGroupModule'] . '.groupID', $groupID);
			$this->where($this->tables['permissionGroupModule'] . '.permission >', 0);
		}

		if ($menuLink != '') {
			$this->where($this->tables['permissionGroupModule'] . '.menuLink', $menuLink);
		}

		//$this->limit(1);
		// $this->orderBy('id', 'desc');

		return $this->permissionGroupsModule();
	}

	/**
	* Permisos de los modulos por grupos
	*
	* @return static
	* @author Chununo
	*/
	public function permissionGroupsModule(): self {
	
		$this->triggerEvents('permissionGroupsModule');

		$builder = $this->db->table($this->tables['permissionGroupModule']);
	...
	/**
	* Obten los permisos del menu
	*
	* @param integer $groupID ID del grupo
	*
	* @return static
	* @author Chununo
	*/
	public function permissionGroupMenu(int $groupID = 0){

		$this->triggerEvents('permissionGroupMenu');

		if ($groupID) {
			$this->where($this->tables['permissionGroupMenu'] . '.groupID', $groupID);
			$this->where($this->tables['permissionGroupMenu'] . '.permission >', 0);
		}

		//$this->limit(1);
		$this->orderBy('menuOrder', 'asc');

		return $this->permissionGroupsMenu();
	}
	...
	* @author Chununo
	*/
	public function permissionGroupsMenu(): self {
	
		$this->triggerEvents('permissionGroupsMenu');

		$builder = $this->db->table($this->tables['permissionGroupMenu']);
	...
	/**
	* Obten los acceso del usario a las sucursales
	*
	* @param integer $userID ID del grupo
	*
	* @return static
	* @author Chununo
	*/
	public function permissionStoreUser(int $userID = 0){

		$this->triggerEvents('permissionStoreUser');

		if ($userID) {
			$this->where($this->tables['permissionStoreUser'] . '.id', $userID);
			$this->where($this->tables['permissionStoreUser'] . '.permission', 1);
		}

		//$this->limit(1);
		// $this->orderBy('id', 'desc');

		return $this->permissionStoresUser();
	}

	/**
	* Accesos permitidos de las tiendas a un usuario
	*
	* @return static
	* @author Chununo
	*/
	public function permissionStoresUser(): self {
	
		$this->triggerEvents('permissionStoresUser');

		$builder = $this->db->table($this->tables['permissionStoreUser']);
